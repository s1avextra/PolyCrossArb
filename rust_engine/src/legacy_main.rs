...
pub use price_state::PriceState;
...
async fn main() {
    ...
    let resub_notify = polymarket_ws::new_subscription_notify();
    {
        let bs = book_state.clone();
        let tt = tracked_tokens.clone();
        let nt = resub_notify.clone();
        tokio::spawn(async move {
            polymarket_ws::polymarket_book_feed(bs, tt, nt).await;
        });
    }
